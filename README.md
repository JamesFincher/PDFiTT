...
- a stateless remote MCP endpoint at `/mcp`
...
Deploy the site to Vercel and connect MCP clients to:
...
`get_setup_instructions` returns Claude Code, Codex, local skill, script, and direct API setup instructions. A normal browser or `curl` GET to `/mcp` also returns the same setup payload:
...
Use it from Codex or any agent runtime that supports local skills by copying that folder into the relevant skills directory. The included `agents/openai.yaml` already points at `https://pdf-i-tt.vercel.app/mcp`.
